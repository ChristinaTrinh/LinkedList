...
public class myArrayListLL_WrapperClasses {
    Node head, tail;
    int myNodeCount;

    // Constructor for objects of class myArrayListLL
    public myArrayListLL_WrapperClasses() {
        head = tail = null;
        myNodeCount = 0;
    }
    ...
    public Node get() { // the client produce the index and we return the object
        Node current;
        // String output = "";
        int selection;
        ...
        current = head;
        int index = 0;
        while (current != null && index < selection) {
    ...
    // Asks the user what they want to add. Returns that object.
    {
        ...
        do {
            ...
            selection = Integer.parseInt(
                    JOptionPane.showInputDialog(null, "Enter Object choice: \n " + choice, "Add: Choose an Object",
                            JOptionPane.INFORMATION_MESSAGE));
        ...
        switch (selection) {
            case 1:
                n = new Node(new Character(value.charAt(0)));
                break;
            case 2:
                n = new Node(new Short(Short.parseShort(value)));
                break;
            case 3:
                n = new Node(new Integer(Integer.parseInt(value)));
                break;
            case 4:
                n = new Node(new Long(Long.parseLong(value)));
                break;
            case 5:
                n = new Node(new Float(Float.parseFloat(value)));
                break;
            case 6:
                n = new Node(new Double(Double.parseDouble(value)));
                break;
            case 7:
                n = new Node(new Boolean(value));
                break;
            default:
    ...
    public void set() {
        ...
        n = addObjectMenu(); // ask for what value the user want to set into

        // remove the current one at the index first
        current = head;
        if (selection == 0)
            head = head.getNext();
        else {
            int index = 0;
            while (current != null && index < selection - 1) {
                current = current.getNext();
                index++;
            }
